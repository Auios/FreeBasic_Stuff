...
//__declspec(dllexport) int __cdecl ccdecl(int) { return 0; }
//#define FnExport __declspec(dllexport) __cdecl

//#define FnExport extern "C"
...
BLFUNC(void, __cdecl, AddStringCommand, Namespace *ns, const char* name, StringCallback cb, const char *usage, int minArgs, int maxArgs);
BLFUNC(void, __cdecl, AddIntCommand, Namespace *ns, const char* name, IntCallback cb, const char *usage, int minArgs, int maxArgs);
BLFUNC(void, __cdecl, AddFloatCommand, Namespace *ns, const char* name, FloatCallback cb, const char *usage, int minArgs, int maxArgs);
BLFUNC(void, __cdecl, AddVoidCommand, Namespace *ns, const char* name, VoidCallback cb, const char *usage, int minArgs, int maxArgs);
BLFUNC(void, __cdecl, AddBoolCommand, Namespace *ns, const char* name, BoolCallback cb, const char *usage, int minArgs, int maxArgs);

//Exposing variables to torquescript
BLFUNC(void, __cdecl, AddVariable, DWORD dictionaryPtr, const char* name, int type, void* data);
...
FnExport bool CompareData(PBYTE data, PBYTE pattern, char* mask)
{
...
FnExport DWORD FindPattern(DWORD imageBase, DWORD imageSize, PBYTE pattern, char* mask)
{
...
FnExport DWORD ScanFunc(char* pattern, char* mask)
{
...
FnExport void PatchByte(BYTE* location, BYTE value)
{
	//Remove protection
	DWORD oldProtection;
...
void ConsoleFunctionString(const char* nameSpace, const char* name, StringCallback callBack, const char* usage, int minArgs, int maxArgs)
{
...
void ConsoleFunctionInt(const char* nameSpace, const char* name, IntCallback callBack, const char* usage, int minArgs, int maxArgs)
{
...
void ConsoleFunctionFloat(const char* nameSpace, const char* name, FloatCallback callBack, const char* usage, int minArgs, int maxArgs)
{
...
void ConsoleFunctionVoid(const char* nameSpace, const char* name, VoidCallback callBack, const char* usage, int minArgs, int maxArgs)
{
...
void ConsoleFunctionBool(const char* nameSpace, const char* name, BoolCallback callBack, const char* usage, int minArgs, int maxArgs)
{
...
const char* Eval(const char* str)
{
	...
	FnExport bool torque_init()
	{
		InitScanner();

		//Printf is required for debug output, so find it first
		Printf = (PrintfFn)ScanFunc((char*)"\x8B\x4C\x24\x04\x8D\x44\x24\x08\x50\x6A\x00\x6A\x00\xE8\x00\x00\x00\x00\x83\xC4\x0C\xC3", (char*)"xxxxxxxxxxxxxx????xxxx");
		
		//Do nothing if we don't find it :(
		if (!Printf)
			return false;

		ShapeNameHudOnRender = (ShapeNameHudOnRenderFn)ScanFunc((char*)"\x81\xec\x00\x00\x00\x00\x53\x8b\xd9\x8a\x83\xc9\x00\x00\x00\x84\xc0\x55\x56\x57\x89\x5c\x24\x14", (char*)"xx????xxxxxxxxxxxxxxxxxx");
		...
		GlobalVars = *(DWORD*)(ScanFunc((char*)"\x8B\x44\x24\x0C\x8B\x4C\x24\x04\x50\x6A\x06", (char*)"xxxxxxxxxxx") + 13);
