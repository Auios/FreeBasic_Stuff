...
input "Name: ", me.sign
'me.sign = GetStr("Name: ",1,1,1)
cls

print "Welcome " & me.sign
print "-----"
print "1. Localhost"
print "2. Cca's Server"
var st = GetStr("Select server: ",1,7,1)
        ...
        if (timer-dSvFps) > 1/svfps then
            with me
                if multikey(sc_lshift) then
                    if multikey(sc_w) then .y-=8: if .y < 0 then OldMe.y += scrny:.y += scrny
                    if multikey(sc_s) then .y+=8: if .y >= scrny then OldMe.y -= scrny:.y -= scrny
                    if multikey(sc_a) then .x-=8: if .x < 0 then OldMe.x += scrnx:.x += scrnx
                    if multikey(sc_d) then .x+=8: if .x >= scrnx then OldMe.x -= scrnx:.x -= scrnx
                else
