...
#define DebugRender() if iDebug then put (v(4),v(2)),pGfx,pset: sleep 100,1
#define DebugRenderTemp() if iDebug then put pGfx,(w(0),h(0)),pTemp,pset:put (v(4),v(2)),pGfx,pset: sleep 100,1
 
dim shared as integer iScale=0,iDay=0,iDebug=0,iTime,iFocus=1
dim shared as fb.image ptr pGfx,pDither,pBirdA,pBirdB,pCollision
dim shared as integer ScrW,ScrH,iHZ,iBest
dim shared as integer AA=1,iFull=0,iDither=1,iSync=2,iShowCrash=0
...
function SetPalette(iDay as integer=-1,iMinuteOfDay as integer=0,iBird as integer=0) as integer
  ...
  if iDay=-1 then
    ...
    if IM > 330 then
      'SetPal( cLight1  ,168,216,184 , 248,184,  8 )
      palette cLight1 ,248,184,  8
    else
      SetPal( cLight1  ,168,216,184 ,   4, 64, 72 )      
    end if
    if IM > 270 then
      'SetPal( cLight2  ,168,216,184 , 248,184, 68 )
    ...
    do
      ...
      case _ARC    'Draws an outlined partial circle X,Y,RADIUS,COLOR,START,END
        ...
        if iScale=1 then
          ...
          #else
            circle pTemp,w(P(1)),h(P(2)),abs(v(P(3))),P(4),fAngS,fAngE,1
          #endif
        else        
          for iSz as single = -iScaleB to -1 step 1
            ...
            #else
              circle pTemp,(w(P(1)),h(P(2))),abs(v(P(3)))+iSz,P(4),fAngS,fAngE,1
              circle pTemp,(w(P(1))-1,h(P(2))),abs(v(P(3)))+iSz,P(4),fAngS,fAngE,1
              circle pTemp,(w(P(1)),h(P(2))-1),abs(v(P(3)))+iSz,P(4),fAngS,fAngE,1
            #endif          
          next ISz          
        end if
        CNT += 7
      ...
      case else
        #ifdef Messagebox
        Messagebox(null,"Bad Parameters","Bad Paramaters",MB_SYSTEMMODAL or MB_ICONSTOP)
        #endif
        stop
      end select      
...
sub DrawBird(iPX as integer,iPY as integer,iAngle as integer,iWing as integer,iColor as integer,pTarget as fb.image ptr=0)
  ...
  if iInit=0 then
    ...
    for CNT as integer = 0 to ubound(iBird)      
      ...
      case else
        #ifdef Messagebox
        Messagebox(null,"Bad Parameters2 " & iBird(CNT),"Bad Paramaters2",MB_SYSTEMMODAL or MB_ICONSTOP)
        end
        #endif
      end select
    next CNT
  ...
  if iAngle <> *piLastAng or iWing <> *piLastWing then
    *piLastAng = iAngle: *piLastWIng = iWing
    ...
    for CNT as integer = 0 to ubound(iBird)
      #ifdef UseFloats
        var fTempAng = csng(fBird(CNT+1)+iAngle*PImul)
        var iX = (iPivX+(cos(fTempAng)*fBird(CNT+2)))
        var iY = (iPivY+(sin(fTempAng)*fBird(CNT+2)))
      #else
        var fTempAng = csng((iBird(CNT+1)+iAngle)*PImul)      
        'var iX = iBird(CNT+1)*iScale+iScaleB, iY = iBird(CNT+2)*iScale+iScaleB
      ...
      case _ARC
        ...
        if iScale=1 then
          ...
          #else
            circle pBird,(iX,iY),iBird(CNT+3),iPal(iBird(CNT+4)),fAngS,fAngE,1
          #endif
        else        
          for iSz as single = -iScaleC to iScaleD step 1
            ...
            #else
              circle pBird,(iX,iY),iBird(CNT+3)+iSz,iPal(iBird(CNT+4)),fAngS,fAngE,1
              circle pBird,(iX-1,iY),iBird(CNT+3)+iSz,iPal(iBird(CNT+4)),fAngS,fAngE,1
              circle pBird,(iX,iY-1),iBird(CNT+3)+iSz,iPal(iBird(CNT+4)),fAngS,fAngE,1
            #endif          
          next ISz          
        end if
        CNT += 6
      ...
      'screenunlock      
      'put (0,0),pBird,pset: sleep 500,1
      'if multikey(1) then stop
      'screenlock
    next CNT    
  end if  
  'Put to target
...
sub SaveConfig()
  ...
  if open(exepath+"/FlappyFB.cfg" for binary access write as #f)=0 then
    ...
    put #f,,iSync: put #f,,iShowCrash: put #f,,iScale\AA
    close #f
  end if
end sub
sub LoadConfig()
...
do  
  ...
  do
    ...
    case fb.EVENT_KEY_PRESS
      ...
      case -fb.SC_F1: iDither xor= 1
      case -fb.SC_F2
        iSync = (iSync+1) mod 5
        dPause = dtimer-dStart: dtimer = timer
        dStart = dtimer-dPause      
        iLastFps=iHz\2:iFps=0:dFps=timer      
        draw string (w(64),h(230)),"Vsync: " & iSync,cRed      
      case -asc("k"),-asc("a"),-fb.SC_F9 'Button
        exit do,do
      case -fb.SC_UP: iPress = 1        
      end select  
    case fb.EVENT_MOUSE_BUTTON_PRESS: iPress = 1            
    case fb.EVENT_WINDOW_CLOSE: exit do,do            
